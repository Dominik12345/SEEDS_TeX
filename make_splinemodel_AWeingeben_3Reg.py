...
for i in range(0,len(hi_knots)):
   hilf = hi_knots[i]
   xi.append('xi'+ str(hilf))
   theta.append('theta'+ str(hilf))
   str_xi = str_xi + '\t\t' + xi[i] + ' = ' +  input(xi[i]+': ') + '\n' # + ' = ' + xi[i] + 'Init\n'
   str_theta = str_theta + '\t\t' + theta[i] + '\n'

for i in range(0,len(hi_knots)):
   hilf = hi_knots[i]
   eta.append('eta'+ str(hilf))
   str_eta = str_eta + '\t\t' + eta[i]+ ' = ' +  input(eta[i]+': ') + '\n'# + ' = ' + eta[i] + 'Init\n'
...
for i in range(0,len(xi)):
	str_objective = (str_objective + 'alpha1 * ' + 'abs(' + xi[i] + ') + '+ 'alpha2/2. * ' + '(' + theta[i] + ')**2 + '  + 'alpha3/2. * '  + '(' + xi[i] + ')**2')   
	if i != (len(xi)-1):
