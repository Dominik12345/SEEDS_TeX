...
datanom.pop(0) #delete first element
...
for i in range(0, len(keys)):
    if (str('w') + str(i) in keys):
        w.append(str('w') + str(i))
...
for i in range(1,len(y)+1):
    ...
    ax.scatter(xt,data_dic['y' + str(i) + 'obs'],c='k',label = 'y' + str(i) + 'obs',s=1.5)
...
fig3 = plt.figure() #in der Schleife mit Abfragen, wann die jeweiligen w 0 sind
quad = np.ceil(np.sqrt(len(wtrue(time))))
for i in range(1,len(wtrue(time))+1):
    ax = plt.subplot(quad,quad,i)
    if isinstance(data_dic['w' + str(i) + 'true'],int) == False:
        ax.plot(xt,data_dic['w' + str(i) + 'true'],c='b',label = 'w' + str(i) + 'true')
    else:
        ax.plot(xt,np.repeat(0,len(xt)),c='b',label = 'w' + str(i) + 'true')
    if 'w' + str(i) in w:
        ax.plot(xt,data_dic['w' + str(i)],c='r',label = 'w' + str(i))
    else:
        ax.plot(xt,np.repeat(0,len(xt)),c='r',label = 'w' + str(i))
    ax.legend()

fig3.tight_layout()
pdfdirectory = 'plot_w.pdf'
pgfdirectory = 'plot_w.pgf'
