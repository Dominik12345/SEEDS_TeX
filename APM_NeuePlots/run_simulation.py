...
def observationnoise(x):
   ret = []
   for i in range(1,len(observation(x0))+1):
       if i in noise:
           i = observation(x)[i-1] + random.uniform(-noisesd,noisesd)
       else:
           i = observation(x)[i-1]
       ret.append(i)
...
data = [ [-1] * (len(observationnoise(x0)) + 2 )  ]
for i in range(0,len(solution)-1):
	temp = observationnoise(solution[i])
	data.append([time[i]] + temp + [0] )
data.append([time[len(solution)-1]] + observationnoise(solution[ len(solution)-1 ]) + [1])
